...
- **Notificacion para calificar (`CalificarService.java`):** depende de que Spotify tenga prendido *Device Broadcast Status*. Califica con el flujo completo, igual que el widget del sidebar, o sea **no sirve para la cola de `/backfill`**. Portada y nota salen de `GET /tracks/info/{id}`, **no** de `/now-playing`: la API de Spotify va detras del celular. Se prueba sin musica simulando el broadcast: `adb shell am broadcast -a com.spotify.music.metadatachanged --es id spotify:track:<id> --es track ... --es artist ...`. **Nunca picar un boton en pruebas**: escribe una calificacion real.
