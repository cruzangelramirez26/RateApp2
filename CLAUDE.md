...
- **Notificacion para calificar (`CalificarService.java`):** depende de que Spotify tenga prendido *Device Broadcast Status*. Son **dos notificaciones** a proposito: la del servicio ("Atento a Spotify", canal minimo, con "Apagar") y la de calificar, que es **normal** para poder irse sola y deslizarse (Android ignora el `cancel` de la notificacion de un servicio). Una linea y las 7 notas; al tocar una sale "Calificada B+ · Deshacer" y **la nota se manda a los 4 s** (Deshacer la cancela antes: no hay endpoint de deshacer). Califica con el flujo completo, igual que Calificar, o sea **no sirve para la cola de `/backfill`**. La nota que ya tiene sale de `GET /tracks/info/{id}`, **no** de `/now-playing`: la API de Spotify va detras del celular. Se prueba sin musica simulando el broadcast: `adb shell am broadcast -a com.spotify.music.metadatachanged --es id spotify:track:<id> --es track ... --es artist ...`. **Nunca picar un boton en pruebas**: escribe una calificacion real.
