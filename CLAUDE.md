...
- **App de escritorio (Tauri):** el instalador se genera con `npm run build` dentro de `desktop/`, y ese script lo copia **siempre** a `instalador/` en la raiz del proyecto (`desktop/copiar-instalador.mjs`). Es donde Angel lo busca. La carpeta esta en `.gitignore` y `.dockerignore`. Si se compila por otro camino, correr `npm run instalador` para copiarlo.
- **App de Android (Capacitor, `mobile/`):** mismo esquema que Tauri, la WebView carga la URL de Cloud Run. `npm run build` dentro de `mobile/` compila y deja `instalador/RateApp.apk`. Se compila con un **JDK 21 portatil** en `AppData\Local\rateapp-jdk` (el Java 25 de Android Studio no corre Gradle 8.14) y la salida de Gradle va a `AppData\Local\rateapp-android-build` (fuera de OneDrive); `mobile/gradle.mjs` pone todo eso. Instalar en el S24 de Angel: `adb install -r --user 0 instalador/RateApp.apk` (el `--user 0` hace falta: la Carpeta Segura es otro usuario). La WebView del APK debug se inspecciona con `adb forward tcp:9444 localabstract:webview_devtools_remote_<pid>`.
- **Notificacion para calificar (`CalificarService.java`):** depende de que Spotify tenga prendido *Device Broadcast Status*. Es **una sola notificacion**, la del servicio, que cambia de estado: una linea y las 7 notas con cada cancion; al tocar una, "Calificada B+ · Deshacer" y **la nota se manda a los 4 s** (Deshacer la cancela antes: no hay endpoint de deshacer); luego una linea "✓ B+ · cancion". **No usar dos notificaciones:** Android 16 / One UI las junta en un grupo "RateApp" contraido que esconde las notas (paso el 2026-09-26). Por lo mismo los avisos caducan a las 12 h. Califica con el flujo completo, igual que Calificar, o sea **no sirve para la cola de `/backfill`**. La nota que ya tiene sale de `GET /tracks/info/{id}`, **no** de `/now-playing`: la API de Spotify va detras del celular. Se prueba sin musica simulando el broadcast: `adb shell am broadcast -a com.spotify.music.metadatachanged --es id spotify:track:<id> --es track ... --es artist ...`. **Nunca picar un boton en pruebas**: escribe una calificacion real.
- **Avisos (`Avisos.java`, fase 5):** tu cola (cada 3 dias a las 10:00, si hay pendientes), sin nota (5 escuchas en 30 dias, una vez por cancion, a las 20:00; lo ya avisado vive en la tabla `avisos_sin_nota`, via `POST /tracks/avisos/sin-nota`) y cierre de cuatrimestre (1 de ene/may/sep, reintenta hasta el 7). Alarmas inexactas: llegan hasta 1 h tarde. Cada uno con su canal. Se prueban sin esperar, solo en el APK de depuracion: `adb shell am start -n com.angelrg.rateapp/.MainActivity --es prueba_aviso cola|sin_nota|cierre` (`sin_nota` consulta con `marcar=false`, no gasta el aviso). Los botones abren la app en una ruta (`--es ruta /dashboard`), que `MainActivity` valida.
- **Widgets (`Widgets.java`, fase 6):** 4x2 lo que suena, 4x1 solo notas, 2x2 tu cola. **No saben que suena por su cuenta:** `CalificarService` les copia cada cambio de estado (SharedPreferences `widgets` + portadas en `filesDir`), asi que dicen lo mismo que la notificacion. Con el servicio apagado muestran "Lo ultimo que sono", y tocar una nota lo **prende** (`getForegroundService`) y califica la cancion que viaja en el intent, no la que suene ahora. Mismo Deshacer de 4 s y mismo flujo completo (no sirven para `/backfill`). El estado se lee con `adb shell run-as com.angelrg.rateapp cat shared_prefs/widgets.xml`. **Ojo al simular el broadcast:** los textos con espacios van entre comillas simples dentro de un solo `adb shell "..."`, y despues hay que regresar al servicio la cancion real, porque un nombre falso se mandaria a MySQL si Angel califica desde la notificacion.
...
- **Mismo umbral de 30 s** que el agregado, para que las dos cifras midan lo mismo. **PERO no cuadran exacto, y los eventos son los buenos:** la PK colapsó **917 duplicados exactos** del export (mismo `track_id`, mismo segundo, hasta 4 veces — solapamiento entre `2019.json`/`2019_1.json`/etc.), que `listening_stats.plays` sí cuenta. Medido en producción el 2026-09-07: agregado **118,869** vs eventos **117,827**, o sea el agregado viene inflado ~0.8%. No es un bug: si los dos números se ven distintos, es por esto.
- **Hueco conocido: 2 → 6 de septiembre de 2026, ~125 reproducciones.** El export llega al 1 de sep y el código que guarda la serie se desplegó el 6, así que esos días están en el agregado pero no en los eventos. Irrecuperable (`recently-played` solo da las últimas 50), pero **se autocorrige**: sale solo de la ventana de 30 días conforme pasa el tiempo.
