...
def rebuild_playlist(cuatri: str):
    """
    Rebuild a cuatrimestre playlist from DB records for the most recent occurrence
    of that period (filtered by year to avoid pulling songs from past years).
    """
    ...
    # Determine the year of the most recent completed (or current) period
    if now.month >= start_m:
    ...
    df["year"] = df["added_at_dt"].dt.year.fillna(0).astype(int)
    ...
    # Only include tracks with a valid non-D rating
    valid_rating = df["rating_order"].notna() & (df["rating_order"] > 0)
    df_cuatri = df[mask & valid_rating].copy()
    ...
    return {
        "ok": True,
        "count": len(track_ids),
        "message": f"{label} reconstruida con {len(track_ids)} canciones (año {target_year}).",
    }
